 ...
 * thread (AudioWorkletGlobalScope) instead of the main thread, avoiding
 * the jitter/dropouts a ScriptProcessorNode-based approach caused by
 * having to fight React's renders for main-thread time.
 *
 * The classes below (FifoSampleBuffer through WebAudioBufferSource) are
 * vendored verbatim from npm's soundtouchjs@0.3.0 (dist/soundtouch.js) -
 * only the pure-JS DSP pieces, not its PitchShifter/getWebAudioNode
 * helpers, which are ScriptProcessorNode-specific and unused here.
 * Inlined (no `import`) rather than fetched from a second file: some
 * browsers' AudioWorklet implementations are unreliable about ES module
 * `import` inside a worklet module, and dropping it removes that failure
 * mode entirely along with an extra network request.
 * To update: `npm view soundtouchjs version`, then re-copy the relevant
 * classes from node_modules/soundtouchjs/dist/soundtouch.js over this
 * section (everything up to, and including, WebAudioBufferSource).
 *
...
// --- End vendored SoundTouch DSP. Below is this app's own processor. ---
